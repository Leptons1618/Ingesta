...
| 3 | Database | `components/connections/database-step.tsx`, `components/connections/database-tree.tsx`, `components/database-connection-form.tsx`, `app/api/local-services/route.ts`, `lib/storage.ts` | Lists saved profiles to pick from (or an empty state that opens the create dialog), then opens `DatabaseTree` for the chosen one: databases expand to their tables, a database can be created in place, and the chosen database is what stage 4 receives. The form offers a local tab (detected services, `POST /api/local-services`) and a remote tab (credentials, validated on test). |
